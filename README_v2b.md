...
<b>This script will take the output csv table of mergeQCresults_plusMissing.py or mergeQCresults_plusMissing_v3.py (or _v5b.py) as input, and:</b>
...
Note: original script calls less VoC/VOIs (B.1.1.7, B.1.351, P.1, B.1.525); v2 and v2b script will call more (see those listed below). 


v2b script will Populate the <b>VariantYesNo column</b> with either Yes, Yes (VOI), No, Failed, Failed (Excess_Ambiguity), Possible, Warning.
    ...
    Populates column with Yes, if: lineage matches those listed in the Positive_values variable (line 51) (['B.1.1.7', 'B.1.351', 'P.1', 'B.1.617', 'B.1.617.1', 'B.1.617.2', 'B.1.617.3'])     {you can add more lineages to the Positive_values variable}
    
    Populates column with Yes (VOI), if: lineage matches those listed in the VUI_Values variable (line 52) (['B.1.427', 'B.1.429', 'B.1.525', 'B.1.526', 'B.1.526.1', 'B.1.1.318', 'P.1.1', 'P.2', 'P.3', 'A.23.1', 'A.27'])     {you can add more lineages to the VUI_Values variable}
    
    Populates column with Yes (Other variant of interest), if: lineage matches those listed in the Other_values variable (line 53) (['B.1.618'])
...
v2 script will Populate the <b>VariantType column</b> with either: [VoC/VoI/Other lineage], Not a VoC, Failed WGS QC, 'Possible Pangolin error, new VOI, or Contamination with [lineage] and ['watchlist_id']', Possible [VoC/VoI/Other lineage]. 
    ...
    (if VariantYesNo = Yes (if lineage belongs to a VoC or VOI/Other lineage)): 'UK (B.1.1.7)','SA (B.1.351)','Brazil (P.1)','India (B.1.617)','India (B.1.617.1)','India (B.1.617.2)','India (B.1.617.3)','Nigerian VOI (B.1.525)','California VOI (B.1.427)','California VOI (B.1.429)','New York VOI (B.1.526)','B.1.526.1 (VOI)','P.1.1 (VOI)','P.2 (VOI)','P.3','B.1.1.318 (VOI)','A.23.1 (VOI)','A.27 (VOI)','B.1.618 (Other - triple mutant)'  
                  ...
                  {note that you can add/remove VoCs and VoIs that you monitor for this - modify the Positive_values variable (line 51) or VUI_Values (line 52), or Other_values (line 53), and the conditions for VariantType (starting at line 103) in the script (---ADD VARIANT COLUMNS to QC Summary Table---- section)}
    ...
    (if VariantYesNo = Warning): 'Possible Pangolin error, new VOI, or Contamination with [lineage] and ['watchlist_id']' (e.g. Possible Pangolin error, new VOI, or Contamination with B.1.1 and P.1)  
...
The script treats these differently. It only pulls out the sampleID for samples (to populate sample column only). Controls are left as-is. FastqIDs are left as-is in sample_name column. 
